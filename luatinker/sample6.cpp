...
public:
	...
	int TestFunc(lua_State* L)
	{
		printf("# TestClass::TestFunc 董界\n");
		return lua_yield(L, 0);
	}

	int TestFunc2(lua_State* L, float a)
	{
		printf("# TestClass::TestFunc2(L,%f) 董界\n", a);
		return lua_yield(L, 0);
	}
};
...
int main()
{
	...
	// 뭐芚변鑒
	lua_tinker::class_con<TestClass>(L,lua_tinker::constructor<TestClass>);
